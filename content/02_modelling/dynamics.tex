...
\end{figure}
On figure \ref{fig:zoom_step} it is seen that the coupling between pan and tilt is insignificant, this means that the system can be decoupled such that the pan does not affect the tilt and visa versa. This conclusion make the state-space model a major overkill as a simple transfer functions can de derived by equation \ref{eq:model_model} the following way:
\begin{align}
