...
\begin{figure}[htb]
	...
	\end{center}
\end{figure}
Figure \ref{fig:control_sketch} show a system with a feedback loop, $G(s)$ is the model of the system itself and $H(s)$ is the regulator. When designing a regulator a system ($H(s)$) is designed. As the system is SISO a PID regulator can be designed.
...
\end{equation}
where $K_p$, $K_i$, and $K_d$ are constants for the proportional, integral, and differential term in the equation. Making the LaPlace-transform leads to:
\begin{equation}
	H(s) = E(s)(K_p + \frac{K_i}{s} + K_d s)
\end{equation}
For our purpose it is necessary to decide which parameters are needed and the magnitude of those. In simulink the following decoupled model have been set up:
\begin{figure}[htb]
	\begin{center}
	\includegraphics[scale=1,trim=0 0 0 0]{graphics/Simulink.pdf} %trim=l b r t (can cut off from every side)
	\caption{This figure illustrates the simulink model.}
	\label{fig:control_sketch}			% figure labels are of the form \label{fig:*}
	\end{center}
\end{figure}
The top system represent the pan part of the system, while the lower part represent the tilt part.
