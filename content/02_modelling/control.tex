...
Since the system is decoupled, it can be represented as shown in \ref{fig:control_sketch} \begin{figure}[htb]
	\centering
...
When run the control task saves the value of the tick counter and when finished, it uses the FreeRTOS \texttt{vTaskDelayUntil} API to calculate when to unblock. As a starting point the control task runs at one hundred hertz frequency as verified in \ref{subsec:control_freq}.
